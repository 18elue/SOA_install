...
def init():
    ...
    domain_template = '%s/wlserver_10.3/common/templates/domains/wls.jar' % BEAHOME
    print 'Reading in the WebLogic Server 10.3 domain template: ' + domain_template
...
def set_server(names):
    ...
    for ms in  MANAGED_SERVER.split(','):
        ...
        if ms_machine:
            if ms_machine not in machine_status:
                print 'creating UnixMachine: ' + ms_machine
                machine_status[ms_machine] = create(ms_machine, 'UnixMachine')
            managed_server.machine = machine_status[ms_machine]
        if ms_cluster:
